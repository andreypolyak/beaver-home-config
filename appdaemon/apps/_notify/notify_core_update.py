...
class NotifyCoreUpdate(hass.Hass):

  def initialize(self):
    self.persons = self.get_app("persons")
    self.storage = self.get_app("persistent_storage")
    self.storage.init("notify_core_update.notified_version", None)
    self.listen_state(self.on_core_update, "sensor.ha_info", attribute="all")
    self.check_updates()
  ...
  def check_updates(self):
    ...
    notified_version = self.storage.read("notify_core_update.notified_version")
    ...
    self.persons.send_notification("admin", message, "core_update", url="/hassio/dashboard")
    self.storage.write("notify_core_update.notified_version", new_version)
