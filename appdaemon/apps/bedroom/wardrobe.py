...
class Wardrobe(Base):
  ...
  def change_wardrobe(self, kwargs):
    self.cancel_handle(self.handle)
    is_door_open = self.entity_is_on("binary_sensor.bedroom_wardrobe_door")
    if is_door_open and self.entity_is_off("light.bedroom_wardrobe"):
      self.turn_on_entity("light.ha_template_individual_bedroom_wardrobe")
    elif not is_door_open and self.entity_is_off("light.group_bedroom_top"):
